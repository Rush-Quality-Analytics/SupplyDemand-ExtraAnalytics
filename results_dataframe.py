...
# IMPORT DATA
seir_fits_df = pd.read_csv('SEIR-SD_States.txt', sep='\t')
StatePops = pd.read_csv('StatePops.csv')
    
cases_df = pd.read_csv('COVID-CASES-DF.txt', sep='\t') 
...
models = ['logistic', 'SEIR-SD', 'exponential', '3rd degree polynomial', 'quadratic']
...
for focal_loc in locations:
    ...
    for model in models:
        ...
        for i, j in enumerate(list(range(ran_len, 1))):
